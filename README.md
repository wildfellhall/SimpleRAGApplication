...
The latest teacher message occupies its own final user turn. Retrieved records are labelled reference data in the system context, so the model is instructed to perform the latest task rather than repeatedly summarize the same evidence. Earlier turns supply references and constraints; a request to revise one exercise should focus on that exercise.
...
A lexical repetition check compares substantive replies with the three most recent assistant answers, ignoring citation and formatting differences. Identical or nearly copied replies receive a targeted retry that redirects the model to the latest task. Explicit repetition, an identical question, short factual replies, summaries, and concrete requested edits are allowed. Both attempts are token-budgeted; discarded answers never enter the visible stream or answer cache. This check catches copying, not every possible paraphrase or off-topic answer.
...
The backend tests exercise real persistent Chroma with deterministic test embeddings and mocked generation. They cover question-to-vector propagation, metadata filters, numerical aggregates, follow-up scope, guided accuracy ranges, no-match behavior, cache invalidation, citation cleanup, migration preservation, timing backfill idempotence, exact duration totals/averages, timing metadata and prompt propagation, failed-rebuild recovery, multi-turn scope retention, token-budget compaction, hidden reasoning, invalid-answer recovery, repeated-answer detection, latest-request prompt placement, and uncached fallbacks. Browser scripts cover desktop/mobile layouts, accessibility, real local-model generation, evidence inspection, guided follow-ups, full-history transport, and retry behavior. `python -m scripts.check_conversation` exercises five consecutive real-model turns including lesson refinement, constraint recall, and a student change. `python -m scripts.check_followups` checks an exit ticket, a focused revision, and a parent note within one conversation. These checks verify application behavior; they are not a model-quality benchmark.
