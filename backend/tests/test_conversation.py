...
def test_invalid_attempt_is_retried_without_leaking_to_stream(monkeypatch,first):
    ...
    assert all(r['max_tokens']==1200 and r['reasoning_format']=='deepseek' and r['chat_template_kwargs']['preserve_thinking'] is False for r in requests)
...
def test_context_denial_retries_and_preserves_teacher_constraints(monkeypatch):
    ...
    history=[{'role':'user','content':'Help Julian with fractions. Ten minutes, no calculators.'},{'role':'assistant','content':GOOD}]
    with TestClient(app) as client:
