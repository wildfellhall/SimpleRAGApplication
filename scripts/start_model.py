...
args = [server,'-m',str(model),'--host','127.0.0.1','--port',os.getenv('MODEL_PORT','8091'),'-c','8192','-np','1','-ngl','99','-t','6','-b',os.getenv('MODEL_BATCH','512'),'-ub',os.getenv('MODEL_UBATCH','256'),'--jinja','--reasoning','off','--reasoning-format','deepseek','--chat-template-kwargs','{"enable_thinking":false,"preserve_thinking":true}','--no-webui','--cache-ram','128','--alias','qwen-local-27b']
