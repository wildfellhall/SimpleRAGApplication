...
type Stats = { problems:number; correct:number; accuracy:number; attempts:number; hints:number; confusion:number; determination:number; confidence:number; frustration:number }
type Student = Stats & {id:string; name:string; grade:number}
type Skill = Stats & {id:string; name:string}
type RecordRow = {id:string; problem_id:string; prompt:string; answer:string; occurred_at:string; correct:boolean; attempts:number; hints:number; skills:string[]; confusion:number; determination:number; confidence:number; frustration:number}
...
type Retrieval = {query:string; retrieval_ms:number; matches:{id:string;score:number}[]}
type Metrics = {cached:boolean; total_ms:number; first_token_ms:number|null}
type ChatMessage = {retrieval?:Retrieval; metrics?:Metrics; role:'user'|'assistant'; content:string; sources?:Source[]; model?:string; truncated?:boolean}
...
function App(){
  ...
  async function send(question=query,chosenFilters?:InsightFilters){
    if(!question.trim()||busy)return
    const filters=chosenFilters??appliedFilters
    setBuilderOpen(false)
    if(chosenFilters)setAppliedFilters(chosenFilters)
    const previous=(chosenFilters?[]:messages).slice(-4).map(({role,content})=>({role,content:content.slice(0,3000)}))
    const assistantIndex=messages.length+1
    setMessages(m=>[...m,{role:'user',content:question.trim()},{role:'assistant',content:''}]);setQuery('');setError('');setBusy(true);setView('chat')
    ...
    try {
      await streamChat({question:question.trim(),student_id:studentId||null,history:previous,filters},({event,data})=>{
        if(event==='retrieval'){
          ...
          setMessages(m=>m.map((message,i)=>i===assistantIndex?{...message,content:message.content+String(data.text)}:message))
        }else if(event==='done'){
          setMessages(m=>m.map((message,i)=>i===assistantIndex?{role:'assistant',content:String(data.answer),sources:data.sources as Source[],model:String(data.model),truncated:Boolean(data.truncated),retrieval:data.retrieval as Retrieval,metrics:data.metrics as Metrics}:message))
        }
      })
    }catch(e){setError(e instanceof Error?e.message:'Unable to generate an answer.');setMessages(m=>m.filter(message=>message.role!=='assistant'||message.content.length>0))}finally{setBusy(false)}
  }
  ...
  return <div className="app-shell">
    ...
    {(source||about)&&<div className="modal-backdrop" onClick={()=>{setSource(null);setAbout(false)}}><section ref={dialog} role="dialog" aria-modal="true" aria-label={source?'Retrieved evidence':'About Forma'} className="modal" onClick={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key==='Escape'){setSource(null);setAbout(false)}}}><button autoFocus className="modal-close" aria-label="Close dialog" onClick={()=>{setSource(null);setAbout(false)}}><X size={20}/></button>{source?<><div className="eyebrow">RETRIEVED EVIDENCE · {source.id}</div><h2>{source.title}</h2><p className="muted">{source.cited?'Cited in this answer.':'Provided to the model as context.'} Values below come directly from the database.</p><pre>{JSON.stringify(source.data,null,2)}</pre></>:<><div className="welcome-emblem"><Layers3 size={25}/></div><h2>A clearer picture of learning.</h2><p>Forma is a local learning assistant built around {overview?.students.length ?? 0} fictional students, {overview?.problem_count ?? 0} problems, {overview?.skills.length ?? 0} skills, and {overview?.summary.problems.toLocaleString() ?? 0} problem sessions.</p><p>Answers use retrieved Chroma records and a model running on this device. Correctness means the final outcome of a problem session. Affect scores are synthetic observations on a 0–1 scale, not psychological assessments.</p><p>Dataset: {overview?.date_range ?? "Loading…"}. Conversations are held in memory and cleared on reload or when switching students.</p><div className="about-tech"><ShieldCheck size={17}/> Local inference · No cloud API</div></>}</section></div>}
  </div>
}
export default App
